            ...
            {job.runs.map((r) => (
              <div key={r.id} className="flex flex-wrap items-center gap-x-3 gap-y-1 rounded-lg border border-slate-100 px-3 py-2 text-[11px] dark:border-slate-800">
                ...
                <span className="ml-auto flex items-center gap-2">
                  ...
                  {r.status === "success" && <button onClick={() => { if (confirm("Restore?")) api(`/api/db/runs/${r.id}/restore`, "POST"); }} disabled={busy} className="text-amber-600 hover:underline disabled:opacity-50">Restore</button>}
                </span>
              </div>
            ))}
          </div>
