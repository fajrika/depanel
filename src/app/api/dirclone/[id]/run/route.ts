...
import { runCloneJob } from "@/lib/dirclone";
...
export async function POST(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const activeRun = await prisma.dirCloneRun.findFirst({ where: { jobId: id, status: "running" } });
  if (activeRun && Date.now() - new Date(activeRun.startedAt).getTime() < 6 * 60 * 60 * 1000) {
  ...
  await logActivity({
    ...
    userId: user.id,
    action: "dirclone-run",
    message: `Jalankan clone "${job.name}"`,
  });
  // fire-and-forget: progress terekam di DirCloneRun
  void runCloneJob(id, "manual").catch((e) => {
    console.error(`[DIRCLONE] Job "${job.name}" gagal dijalankan: ${(e as Error).message}`);
  });
